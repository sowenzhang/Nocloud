...
class AppState {
    ...
    // ── Network trust ─────────────────────────────────────────────────────────
    private val _currentNetworkId = MutableStateFlow(detectSsid() ?: detectNetworkId())
    ...
    private val downloadDir = File(System.getProperty("user.home"), "Downloads/NoCloud Chat")
    ...
    init {
        ...
        scope.launch {
            while (isActive) {
                delay(30_000)
                val newNetId = detectSsid() ?: detectNetworkId()
    ...
    fun openLocalFile(path: String) {
        scope.launch(Dispatchers.IO) {
            try { java.awt.Desktop.getDesktop().open(File(path).parentFile) }
            catch (_: Exception) {}
        }
    }
